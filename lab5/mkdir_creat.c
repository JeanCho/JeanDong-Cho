...
int kmkdir(MINODE *pmip, char *n)
{
    ...
    char buf[BLKSIZE];//get block here later

    MINODE *mip = iget(pmip->dev, ino);//allocater
    INODE *ip = &mip->INODE;
    DIR *dp;
    char *cp;
    //new indoe structure pointing memeory inode
    ip->i_mode = 0x41ED;
    ip->i_uid = running->uid;//permission stuff
    ip->i_gid = running->gid;
    ip->i_size = BLKSIZE;
    ip->i_links_count = 2; //. and .. ->itself and parent
    ip->i_mtime = ip->i_ctime = ip->i_atime = time(0L);//time stuff
...
int my_mkdir(char *dName)
{
    ...
    char *parent = dirname(dName);//directory path
    char *child = basename(temp);//name of the file creating

    int pino = getino(parent);//parent inumber
    ...
    {
        if (search(pmip, child) == 0)//make sure that child name is does exist or not
        {
            kmkdir(pmip, child);
